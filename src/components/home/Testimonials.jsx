...
export default function TestimonialCarousel() {
  ...
  const renderStars = (rating) => {
    return Array.from({ length: 5 }, (_, i) => (
      <Star
        key={i}
        className={`w-4 h-4 ${
          i < rating ? "text-[#4F46E5] fill-[#4F46E5]" : "text-neutral-300"
        }`}
      />
    ));
  };
  ...
  return (
    <section className="bg-gradient-to-br from-slate-50 to-white py-18">
      <div className="max-w-6xl mx-auto p-[1rem]">
        ...
        <div className="text-center mb-12">
          <h2 className="text-3xl lg:text-4xl font-bold text-neutral-800 font-display mb-4">
            What Our <span className="text-[#4F46E5]">Users</span> Say
          </h2>
          <p className="text-lg text-neutral-600 max-w-2xl mx-auto">
            Discover why thousands of customers and dealerships trust Amacar
          </p>
          <div className="w-20 h-1 bg-[#4F46E5] mx-auto rounded-full mt-4"></div>
        </div>
          ...
          >
            <CarouselContent className="-ml-2 md:-ml-4">
              {testimonials.map((testimonial, index) => (
                ...
                >
                  <div className="group relative bg-white rounded-2xl p-6 transition-all duration-300 hover:shadow-xl hover:shadow-[#4F46E5]/10 border border-neutral-200 overflow-hidden h-full">
                    {/* Quote Icon */}
            ...
            {/* Navigation Arrows */}
            <CarouselPrevious className="cursor-pointer absolute left-6 top-1/2 transform -translate-y-1/2 -translate-x-6 bg-white rounded-full p-4 transition-all duration-300 border border-neutral-200 hover:bg-[#4F46E5] hover:border-[#4F46E5] hover:text-white shadow-lg" />
            <CarouselNext className="cursor-pointer absolute right-6 top-1/2 transform -translate-y-1/2 translate-x-6 bg-white rounded-full p-4 transition-all duration-300 border border-neutral-200 hover:bg-[#4F46E5] hover:border-[#4F46E5] hover:text-white shadow-lg" />
          </Carousel>
