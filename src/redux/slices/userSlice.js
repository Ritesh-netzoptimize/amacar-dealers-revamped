...
export const verifyOTP = createAsyncThunk(
  ...
  async (data, { rejectWithValue }) => {
    try {
      ...
      if (response.data.success) {
        console.log(
          "OTP verification successful, returning token:",
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (response.data.success) {
        ...
        return rejectWithValue(
          response.data.message || "Failed to fetch billing information"
        );
      }
    } catch (error) {
      console.error(
      ...
      return rejectWithValue(
        error.response?.data?.message || "Failed to fetch billing information"
      );
    }
  }
);
  ...
  async (file, { rejectWithValue }) => {
    try {
      ...
      if (file.size > 2 * 1024 * 1024) {
        return rejectWithValue(
          "File size too large. Please upload images smaller than 2MB."
        );
      }
